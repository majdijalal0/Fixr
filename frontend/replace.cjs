const fs = require('fs');
const content = fs.readFileSync('src/pages/WorkerProfile.jsx', 'utf8');

let newContent = content.replace(
  /import \{\n\s*Star, MapPin, Briefcase, Clock, BadgeCheck,\n\s*Phone, ChevronLeft, Image as ImageIcon, Loader2, MessageSquare, Send\n\} from 'lucide-react';/,
...
const returnEnd = newContent.lastIndexOf('  );\n};\n');

const newReturn = `  return (
    <div className="min-h-screen pt-24 pb-16 bg-slate-50 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-6">
        <button
          onClick={() => navigate(-1)}
          className="inline-flex items-center gap-2 text-slate-500 hover:text-amber-600 font-semibold transition-colors"
        >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-12 gap-8">
          ...
          {/* LEFT: Profile Sidecard (4 columns) */}
          <motion.div
            ...
            <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-sm border border-slate-200/60 overflow-visible relative transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
              <div className="pt-16 pb-8 px-6 text-center">
                <div className="absolute -top-12 left-1/2 -translate-x-1/2">
                  <div className="relative">
                    <img
                      src={avatarUrl}
                      alt={worker.name}
                      className="w-32 h-32 rounded-3xl object-cover ring-4 ring-white shadow-xl rotate-3 transition-transform hover:rotate-0 duration-300"
                    />
                    <div className="absolute -bottom-2 -right-2 bg-white p-1.5 rounded-full shadow-lg">
                      {wp.isVerified ? (
                        <BadgeCheck className="w-6 h-6 text-blue-500" title="Verified" />
                      ) : (
                ...
                <div className="mt-8">
                  ...
                  <div className="inline-flex items-center gap-3 bg-slate-100/80 px-4 py-2 rounded-full border border-slate-200/60 mb-6">
                    <span className="relative flex h-3 w-3">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-3 w-3 bg-emerald-500"></span>
                    </span>
                    <span className="text-sm font-semibold text-slate-700">Live: Currently in Fès</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center bg-slate-50/50 rounded-2xl p-4 border border-slate-100">
                    ...
                    <div className="w-px h-8 bg-slate-200"></div>
                    <div className="text-center">
                      <p className="text-xl font-mono font-bold text-slate-900">{String(wp.experience || '0').replace(/[^0-9]/g, '') || 0}</p>
            ...
            <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-sm border border-slate-200/60 p-6 mt-6 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
               ...
               <div className="space-y-3">
                  ...
                  <a href="#" className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                     ...
                     <span className="font-semibold text-slate-700 text-sm">Servicing Fès Area</span>
                  </a>
               </div>
            </div>
          </motion.div>

          {/* RIGHT: Content Blocks (8 columns) */}
          <motion.div
          ...
          >
            ...
            {/* Bento Grid layout for middle sections */}
...
newContent = newContent.substring(0, returnStart) + newReturn + '\n  );\n};\n';
...
fs.writeFileSync('src/pages/WorkerProfile.jsx', newContent);
